...
| Redirect resolution | Stale up to the cache TTL — nothing invalidates on update or delete |
...
Link is protected by, but does not implement, the platform's access control. The SpiceDB `link` definition —
`reader`, `writer`, `linkteam`, `view = reader + writer + linkteam->view_all_links` — lives in
[[system|permission-system]] in the [[domain|Auth]] domain.

The [[entity|Link]] aggregate additionally carries an email allowlist of its own, where empty means public. Two
independent models describe who may see a link.
